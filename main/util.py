...
def read_enc_emg(emg_path,norm='',context=None):
    emg = np.load(emg_path)
    #emg = np.asarray(m) # get the value(1-d)
    #print("emg original shape:", emg.shape)
...
#     pdb.set_trace()
    ...
    s_pesq = pesq(16000, clean, enhanced)
...
def make_spectrum(filename=None, y=None, is_slice=False, feature_type='logmag', mode=None, FRAMELENGTH=None,
    ...
    D = librosa.stft(y,center=False, n_fft=512, hop_length=128,win_length=512,window=scipy.signal.windows.blackman)
